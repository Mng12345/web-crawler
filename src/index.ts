...
async function main() {
	...
	program
		...
		.option(
			'--base-path <path>',
			'only crawl URLs under this path (e.g. /docs)'
		)
		.option(
			'--domain <domain>',
			'only crawl URLs under this domain (e.g. example.com)'
		)
      ...
      const basePath =
				opts.basePath && opts.basePath.startsWith('/')
					? opts.basePath
			...
			if (!isAllowed(startUrl, basePath, domain)) {
				console.error(
					'起始 URL 不满足 --base-path 或 --domain 限制，程序退出。'
				)
				process.exit(1)
			}
